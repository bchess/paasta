...
def is_healthy_in_synapse(local_port, backends):
...
def synapse_replication_is_low(service, instance, system_paasta_config, local_backends):
    ...
    crit_threshold = 80
    namespace = read_namespace_for_service_instance(service, instance, soa_dir=soa_dir)
    ...
    expected_count = get_expected_instance_count_for_namespace(service, instance, soa_dir=soa_dir)
    expected_count_per_location = int(expected_count / len(smartstack_replication_info))
    full_name = "%s.%s" % (service, namespace)
    ...
    num_available = local_replication.get(full_name, 0)
    under_replicated, ratio = utils.is_under_replicated(
        num_available, expected_count_per_location, crit_threshold)
    if under_replicated is True:
        log.warning('Service %s.%s has %d out of %d expected instances (CRITICAL: %d%%)\n' % (
            service, instance, num_available, expected_count_per_location, ratio))
        return True
    else:
        log.debug('Service %s.%s has %d out of %d expected instances (OK: %d%%)\n' % (
            service, instance, num_available, expected_count_per_location, ratio))
        return False
    return True
...
def are_local_tasks_in_danger():
    try:
        ...
        for service, instance, port in local_services:
            ...
            if is_healthy_in_synapse(port, local_backends) and \
