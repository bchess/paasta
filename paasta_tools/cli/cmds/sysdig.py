...
def paasta_sysdig(args):
    if not args.local:
        ...
        output = _run('ssh -At -o LogLevel=QUIET {0} "paasta {1} --local"'.format(mesos_master, ' '.join(sys.argv[1:])))
        slave, command = output[1].split(':', 1)
        print "ssh -tA {0} '{1}'".format(slave, command.strip())
        sys.exit(0)
